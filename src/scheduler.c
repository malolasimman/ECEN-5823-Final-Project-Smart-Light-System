...
void schedulerSetEventCOMP1(void)
{
  ...
  events |= COMP1bitmask;           // Set COMP1 event
...
void schedulerSetI2CEvent()
{
  ...
  events |= I2C_COMPLETED;          // Set I2C_COMPLETE Event
...
void temperature_state_machine(uint32_t event)
{
  ...
  {
    case IDLE:
      if(event == UF_EVENT)                                           // Under flow event occurred
      {
     ...
     case I2C_COMMAND:
       if(event == COMP1_EVENT)                                       // COMP1 Occurred, elapsed power up time
       {
    ...
    case I2C_WAIT:
      if(event == I2C_COMPLETE)                                        // I2C write command complete
      {
    ...
    case I2C_READ:
      if(event == COMP1_EVENT)                                         // COMP1 Occurred, elapsed conversion time
      {
         sl_power_manager_add_em_requirement(SL_POWER_MANAGER_EM1);    // add em1 power requirement
    ...
    case CONVERT_TEMP_TO_C:
      if(event == I2C_COMPLETE)                                         // I2C read complete
      {
          NVIC_DisableIRQ(I2C0_IRQn);                                   // Disable the I2C0 interrupt
          sl_power_manager_remove_em_requirement(SL_POWER_MANAGER_EM1); // remove em1 power requirement
